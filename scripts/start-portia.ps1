...
Start-Process powershell -ArgumentList "-NoExit", "-Command", "cd '$lexerPath'; Write-Host 'LEXER BACKEND - Port 8000 (watchfiles)' -ForegroundColor Blue; Write-Host 'Watching for *.py and *.lark changes...' -ForegroundColor Yellow; & .venv-py312\Scripts\watchfiles.exe '.venv-py312\Scripts\python.exe -m uvicorn app.main:app --port 8000' ."

Start-Sleep -Seconds 2

Write-Host "Starting Parser Backend with hot-reload..." -ForegroundColor Yellow
Start-Process powershell -ArgumentList "-NoExit", "-Command", "cd '$parserPath'; Write-Host 'PARSER BACKEND - Port 8001 (watchfiles)' -ForegroundColor Magenta; Write-Host 'Watching for *.py and *.lark changes...' -ForegroundColor Yellow; & .venv-py312\Scripts\watchfiles.exe '.venv-py312\Scripts\python.exe -m uvicorn main:app --port 8001' ."
...
Write-Host ""
Write-Host "Hot-reload enabled: Services restart on *.py/*.lark changes." -ForegroundColor Yellow
